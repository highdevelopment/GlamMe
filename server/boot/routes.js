...
module.exports = function(app) {
  ...
  // 대회 정보
  app.get('/contest-info', async(req, res) => {
    const user = await authorize(req, res, false);
    try {
      res.render('contest-info', { 
        user: user
      })
    } catch(e) {
  ...
  // 나의 콘테스트
    ...
    // 대회 상세
    app.get('/contest-detail', async(req, res) => {
      const user = await authorize(req, res, false);
      try {
    ...
    // 대회 정보
    app.get('/contest-detail-info', async(req, res) => {
      const user = await authorize(req, res, false);
      try {
        res.render('contest-detail-info', { 
          user: user
        })
      } catch(e) {
    ...
    // 대회 개요
    app.get('/contest-detail-summary', async(req, res) => {
      const user = await authorize(req, res, false);
      try {
        res.render('contest-detail-summary', { 
          user: user
        })
      } catch(e) {
  ...
  app.get('/my-profile', async(req, res) => {
    try {
      ...
      res.render('my-profile', {
        ...
        mail_id: mailed[0] == 'title="이메일' ? '' : mailed[0],
        mail_type: mailed[1] == 'title="이메일' ? '': mailed[1],
      })
    } catch(e) {
      // return Promise.reject(e);
    }
  ...
  app.get('/test', async(req, res) => {
    try {
      ...
      const result = [];
      const gameObjs = await app.models.Games.find(query);
      await Promise.all(gameObjs.map(async gameObj => {
        await Promise.all(Object.keys(gameObj.scores).map(async playerId => {
          const userObj = await app.models.Accounts.findById(playerId);
          if (userObj) {
            const scores = gameObj.scores[playerId];
            var isValid = true;
            let total_score = 0;
            scores.map(score => {
              if (score == 99) isValid = false;
              total_score += score;
            })
            if (isValid) {
              result.push({
                nickname: userObj.nickname,
                course: gameObj.course_name,
                scores: scores,
  ...
  app.get('/test_page', async(req, res) => {
    try {
      const user = await authorize(req, res, true);
      var result = {
        test: "tessss"
