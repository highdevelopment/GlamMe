...
const ServerConfig = require('./../../server/boot/routes/config.json');
...
module.exports = function(MotionVideo) {
    ...
    MotionVideo.createVideo = async(gameLogId, filepath, filepath_thumb, holeIndex, club, carry, userObj) => {
        ...
        try {
            ...
            const storeObj = await Store.findById(gameLogObj.storeId, {fields: ['id', 'storeName']});
            const data = {
                filepath: filepath,
            ...
            const gameObj = await gameLogObj.game.get();
            const length = await MotionVideo.count();
            const params = {
                nasmoSeq: length,
                userId: userObj.id.toString(),
                gameCd: 100,
                gameLogCd: 100,
                holeType: holeIndex < 9 ? 'IN' : 'OUT',
                holeNum: holeIndex,
                imgUrl: ServerConfig.FILE_SERVER_LOCALPATH + filepath_thumb,
                fileUrl: ServerConfig.FILE_SERVER_LOCALPATH + filepath,
                mUserId: userObj.id.toString(),
                mGameCd: gameObj.id.toString(),
                mGameLogCd: gameLogId,
                flagYN: 'Y',
                issueID: 'Api',
                clubDistance: carry,
                clubNo: club,
                mStoreCd: storeObj.id.toString()
            }
            var response = await request('http://man.doorsgolf.com/webIF/saveNasmo', {
                method: 'POST',
                data: params
            })
