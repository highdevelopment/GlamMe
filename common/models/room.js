...
module.exports = function(Room) {
  ...
  Room.remoteMethod('verifyRooms', {
    accepts: [
      ...
      {arg: 'rooms', type: 'string', required: true, description: 'getNewRoom의 결과에서 rooms값 배열 실례: ["23ds...", "32jk..."]'},
    ],
    description: [
  ...
  Room.changeRoomType = async(roomId, type) => {
    try {
      const query = {
        where: {
          'name': '오신'
      ...
      const roomObj = await Room.findOne(query);
      // const roomObj = Room.findById(roomId);
      const result = await roomObj.updateAttribute('machineType', type)
      return Promise.resolve(Common.makeResult(true, 'success', result));
    } catch(e){
      console.error(e);
      return Promise.reject(e);
  ...
  Room.remoteMethod('changeRoomType', {
    accepts: [
      {arg: 'roomId', type: 'string', required: true, description: 'roomId값'},
      {arg: 'type', type: 'number', required: true, description: '0: 룸형, 1: 타석'},
    ],
    description: [
    ],
    returns: {
    ...
    http: {path:'/change-type', verb: 'post'}
  });
};
