...
module.exports = function(Container) {
    ...
    Container.uploadUserMotions = async(req, res, motions, access_token, gameLogId, holeIndex, club, carry) => {
        try {
            const Accounts = Container.app.models.Accounts;
            if (motions.length < 0) return Common.makeResult(false, 'there is no data');
    ...
    Container.remoteMethod('uploadUserMotions', {
            description: 'Uploads a files',
            accepts: [
                { arg: 'req', type: 'object', http: { source:'req' } },
                { arg: 'res', type: 'object', http: { source:'res' } },
                { arg: 'motions', type: 'object', required: true, description: '모션자료배열' },
            ],
            returns: {
                arg: 'fileObject', type: 'object', root: true
            },
            http: {path:'/upload-usermotions', verb: 'post'}
        }
    );
