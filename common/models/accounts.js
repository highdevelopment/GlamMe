...
module.exports = function(Accounts) {
  ...
  Accounts.remoteMethod('searchUser', {
    accepts: [
      {arg: 'name', type: 'string', description: '빈문자열이면 전체검색, 아이디와 닉네임이 검색문자열을 포함하는 모든 유저를 표시'},
      {arg: 'role', type: 'string', description: '빈문자열이면 전체검색, user / manager / admin, manager이면 role이 매장관리자인 유저만 검색'},
  ...
  Accounts.resetAverageScore = async(userObj, lastGameLog) => {
    try {
      ...
      if (score_count > 0) {
        ...
        if (best_score != 9999) data.user_bestscore = best_score;
      }
      if (context_count > 0) {
...
Accounts.resetAverageScoreAllUser = async(time) => {
  try {
    ...
    //   }
    // }
    // const roomsAll = await Room.find(query3);
    // await Promise.all(roomsAll.map(async roomObj => {
    //   await roomObj.updateAttribute('status', 'deleted');
    // }))
    // return Promise.resolve(Common.makeResult(true, 'success', {
    //   length: rooms.length,
    //   rooms: rooms,
    ...
    //   await roomObj.updateAttribute('playStatus', 0);
    // }))
    ...
    const res_users = await Accounts.find(query);
...
Accounts.remoteMethod('resetAverageScoreAllUser', {
  ...
  http: {path:'/resetAverageScore', verb: 'get'}
});
