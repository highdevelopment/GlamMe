...
module.exports = function(Store) {
  ...
  Store.remoteMethod('saveRoomsOfStore', {
    accepts: [
      ...
      {arg: 'rooms', type: 'array', required: true, description: 'room 배열 [{id: xxx, status: "stopped"}, ...] On일때 waitting, Off일때 stopped'},
    ],
    description: [
  ...
  Store.remoteMethod('getStoreStatistics', {
    accepts: [
      {arg: 'search', type: 'string', required: false, description: '검색문자열, 빈문자열일때 전체검색'},
      {arg: 'date1', type: 'date', required: false, description: '검색구간1, 빈문자열일때 30일전'},
      {arg: 'date2', type: 'date', required: false, description: '검색구간2, 빈문자열일때 현재시간'}
    ],
    description: [
      '(어드민) 사업자별매출관리.\n',
    ],
  ...
  function getRoomEstimateEndTime(gameObj) {
    let result = new Date();
    const now = Date.now();
    if (gameObj.current_hole == 0) {
      result.setTime(gameObj.startedAt.getTime() + gameObj.player_num * 3600 * 1000 * gameObj.game_holenum / 18);
    } else {
      if (gameObj.hole_game_logs && gameObj.hole_game_logs.length > 0) {
        let sum_time = 0;
        let cnt_hole_time = 0;
        for (let i = 0; i < gameObj.hole_game_logs.length; i++) {
          let hole_time_per_user = 3 * 60 * 1000;
          const curLog = gameObj.hole_game_logs[i];
          curLog.time = new Date(curLog.endedAt);
          if (curLog) {
            if (i == 0) {
              hole_time_per_user = (curLog.time.getTime() - gameObj.startedAt.getTime()) / gameObj.player_num;
              cnt_hole_time += (curLog.holeIndex);
            } else {
              const beforeLog = gameObj.hole_game_logs[i - 1];
              if (beforeLog) {
                hole_time_per_user = (curLog.time.getTime() - beforeLog.time.getTime()) / gameObj.player_num;
                cnt_hole_time += (curLog.holeIndex - beforeLog.holeIndex);
              }
            }
          }
          hole_time_per_user = Math.min(Math.max(hole_time_per_user, 2 * 60 * 1000), 7 * 60 * 1000);
          sum_time += hole_time_per_user;
        }
        const aver_hole_time_per_user = sum_time / cnt_hole_time;
        const estimate_time = aver_hole_time_per_user * gameObj.player_num * (gameObj.game_holenum - gameObj.current_hole);
        result.setTime(now + estimate_time);
      } else {
        let pass_holetime = (now - gameObj.startedAt.getTime()) / gameObj.current_hole / gameObj.player_num;
        pass_holetime = Math.min(Math.max(pass_holetime, 2 * 60 * 1000), 10 * 60 * 1000);
        result.setTime(now + (gameObj.game_holenum - gameObj.current_hole) * pass_holetime * gameObj.player_num);
      }
    }
    return result;
  }
  ...
  Store.getManagerGameStatusByRoom = async(access_token) => {
    ...
    try {
      ...
      const rooms = await storeObj.room.find(query);
      const query_game = {
        fields: ['startedAt', 'endedAt', 'game_mode', 'player_num', 'game_staus', 'current_hole', 'game_holenum', 'course_name', 'roomId', 'hole_game_logs'],
        where: {
          game_staus: 0,
          ...
          if (gameObj) {
            ...
            if (roomObj.machineType == 1) {
              ...
              if (gameObj.game_mode == 'driving') {
                ...
                prediction.setTime(getRoomEstimateEndTime(gameObj));
              }
            }
